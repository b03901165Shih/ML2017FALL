...
class ValidCallback(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		y_pred = np.round(model.predict(x_test, batch_size = batch_size, verbose = 1)).reshape(y_test.shape[0],)
		y_true = y
		acc = Acc(y_true,y_pred)
		print('\nTesting acc: {}\n'.format(acc))
		if acc > accurate+0.00001:
...
for k in range(11):
	...
	#index = np.logical_and(np.abs(y_pred_raw-y_pred) > 0.05, np.abs(y_pred_raw-y_pred) < 0.15)
...
model.fit(x_train, y_train,
		  ...
		  callbacks=[ValidCallback((x_test,y_test,accuracy))])
